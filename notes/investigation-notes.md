# Investigation Notes: WMI Permanent Event Consumer Investigation

## Investigation Summary

This investigation analyzed a controlled WMI permanent event subscription created on a Windows 11 endpoint.

The objective was to reconstruct the relationship between a WMI event condition, an event filter, a command-line event consumer, and the binding that connects the two.

The investigation confirmed the presence of the following controlled WMI components:

- `__EventFilter`
- `CommandLineEventConsumer`
- `__FilterToConsumerBinding`

A benign PowerShell payload was configured as the execution action.

The resulting payload output confirmed successful execution under:

```text
NT AUTHORITY\SYSTEM
```

Sysmon Event ID 11 provided evidence of payload file creation, while Sysmon Event ID 1 provided supporting evidence of PowerShell process creation.
...
The strongest evidence relationship was:
...
The resulting execution output established:
...
The investigation demonstrates that analysts should focus on **correlation and execution verification**, not merely the discovery of individual WMI objects.
