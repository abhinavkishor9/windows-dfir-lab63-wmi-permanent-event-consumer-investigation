# Troubleshooting Notes: WMI Permanent Event Consumer Investigation

## Overview

This document records the investigation considerations, observations, and troubleshooting lessons identified during the WMI Permanent Event Consumer Investigation.

The lab successfully created and investigated a controlled WMI permanent event subscription.

The primary challenge was not simply identifying WMI objects, but determining how the objects relate to each other and whether they resulted in actual execution.
...
- Enumerate existing WMI subscriptions before creating controlled artifacts.
- Do not assume every WMI subscription is malicious.
- Investigate filters, consumers, and bindings together.
- Use targeted filtering after identifying relevant WMI objects.
- Verify the referenced payload path.
- Inspect payload contents.
- Hash controlled artifacts where appropriate.
- Correlate Sysmon Event ID 11 with subsequent activity.
- Correlate Sysmon Event ID 1 with the WMI execution chain.
- Determine the actual execution security context.
- Treat `ExecutionPolicy Bypass` as an indicator requiring context.
- Verify actual execution instead of assuming configuration equals execution.
- Preserve all three WMI subscription components.
- Use baseline information to distinguish pre-existing artifacts.
- Do not directly attribute unrelated Wazuh alerts without supporting evidence.
- Combine SIEM telemetry with endpoint-level investigation.

---

## Final Troubleshooting Conclusion

The primary investigation challenge was reconstructing the relationship between multiple WMI artifacts.

A reliable investigation requires correlation across:
...
The lab demonstrated that the strongest evidence comes from combining configuration artifacts with actual execution telemetry.
